// PARA LA ARENA
int maxArena = 150;                  // Cantidad de granos de arena en pantalla
float[] arenaX = new float[maxArena]; // Posiciones X de cada grano
float[] arenaY = new float[maxArena]; // Posiciones Y de cada grano
float[] arenaVel = new float[maxArena]; // Velocidad de cada grano
boolean arenaInicializada = false;    // Para crear los granos una sola vez
float alphaFin = 0;                  // Para la aparición gradual del cartel 
...
void setup() {
  ...
  tiempo = millis(); // Se inicializa el tiempo una sola vez al final del setup
}
...
void pantalla5() {
  ...
  if (!arenaInicializada) {
    for (int i = 0; i < maxArena; i++) {
      arenaX[i] = random(0, width);       
      arenaY[i] = random(0, height);      
      arenaVel[i] = random(4, 10);        
    }
  ...
  for (int i = 0; i < maxArena; i++) {
    arenaX[i] -= arenaVel[i]; 
    arenaY[i] += random(0.5, 1.5); 
    if (arenaX[i] < 0) { arenaX[i] = width; arenaY[i] = random(0, height); }
    if (arenaY[i] > height) { arenaY[i] = 0; arenaX[i] = random(0, width); }
    fill(214, 140, 50, random(40, 130)); 
    float tamano = map(arenaVel[i], 4, 10, 2, 5); 
    ellipse(arenaX[i], arenaY[i], tamano * 2.5, tamano); 
  }
  ...
  // BOTÓN DE REINICIO (Simplificado)
...
void mouseClicked() {
  ...
  if (pantalla == 4 && mouseX > 240 && mouseX < 400 && mouseY > 240 && mouseY < 280) {
    // 1. Volver a la pantalla de menú (0)
    pantalla = 0;
    tiempo = millis();
    
    // 2. Reseteamos Pantalla 1
    yTitulo = -100;
    
    // 3. Reseteamos Pantalla 2 (Furiosa y Max)
    yProta2 = 500;
    yProta = 500;
    alphaFuriosa = 255;
    alphaMax = 255;
    ...
    // 4. Reseteamos Pantalla 3 (Immortan Joe)
    yFondo3 = 480;
    yJoe = 500;
    alphaJoe = 255;
    alphaTexto3 = 0;
    ...
    // 5. Reseteamos Pantalla 4 (Max y Furiosa juntos)
    inicioPantalla4 = 0;
    yMaxFuriosa = 480;
    alphaMaxFuriosa = 0;
    
    // 6. Reseteamos Pantalla 5 (Tormenta y botón)
    alphaFin = 0;
    arenaInicializada = false;
  }
}

// Función limpia dedicada exclusivamente a renderizar el botón de reinicio
